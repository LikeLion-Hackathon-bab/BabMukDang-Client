...
import { useHeaderStore, useMealPlanStore } from '@/store'

const guestSessionStorageKey = (token: string) => `mealPlanGuestSession:${token}`
...
export function MealPlanGuestSessionPage() {
    ...
    if (isLoading) {
        return <div className="py-40 text-center text-gray-5">게스트 밥약을 불러오는 중입니다.</div>
    }
...
function MealPlanGuestSessionContent({ nickname }: { nickname: string }) {
    ...
    if (!mealPlan) {
        return <div className="py-40 text-center text-gray-5">밥약 정보를 준비하는 중입니다.</div>
    }
        ...
        <div className="flex flex-col gap-20 py-20">
            <section className="rounded-20 bg-primary-100 border-primary-300 border p-16">
                ...
                <p className="mt-6 text-caption-regular text-gray-6">
                    ...
                    친구 목록, 프로필 상세, 내 밥그릇 기능에는 접근하지 않습니다.
                </p>
            </section>
            ...
            <section className="rounded-20 bg-white p-16">
                <h2 className="text-body1-semibold text-gray-8">가입하면 더 편해져요</h2>
                <p className="mt-6 text-caption-regular text-gray-5">
                    기록 작성, 반복 참여, 친구 초대를 사용하려면 회원 가입이 필요합니다.
                </p>
                <Link
                    to="/login"
