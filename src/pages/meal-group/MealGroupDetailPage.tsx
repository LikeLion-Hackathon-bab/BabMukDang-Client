        ...
        <div className="flex flex-col gap-20 py-20">
            <section className="rounded-24 bg-primary-100 border-primary-300 border p-18">
                ...
                <p className="text-body2-medium text-gray-6">
                    멤버 {data.members.length}명 · 최근 밥약{' '}
                    {data.recentMealPlanIds.length}개
                </p>
            </section>
            ...
            {preferences && (
                <section className="rounded-20 flex flex-col gap-10 bg-white p-16">
                    ...
                    <p className="text-caption-regular text-gray-5">
                        자주 먹은 메뉴:{' '}
                        {preferences.frequentMenuCategories
                            .map(item => `${item.label} ${item.count}회`)
                            .join(', ') || '아직 없습니다.'}
                    </p>
                    <p className="text-caption-regular text-gray-5">
                        자주 간 식당:{' '}
                        {preferences.frequentRestaurants
                            .map(
                                item => `${item.restaurantName} ${item.count}회`
                            )
                            .join(', ') || '아직 없습니다.'}
                    </p>
                    <p className="text-caption-regular text-gray-5">
                        추천 후보:{' '}
                        {preferences.recommendationContext.candidateMenuCategories.join(
                            ', '
                        ) || '히스토리가 쌓이면 자동 반영됩니다.'}
                    </p>
                </section>
