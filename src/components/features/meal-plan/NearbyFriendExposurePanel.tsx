...
export function NearbyFriendExposurePanel({
    mealPlanId
}: {
    mealPlanId: string
}) {
        ...
        <section className="rounded-20 flex flex-col gap-12 bg-white p-16">
            <div>
                ...
                <p className="text-caption-regular text-gray-5">
                    명시 동의, 친구 관계, 위치 조건, 공복 상태를 만족한
                    친구에게만 노출됩니다.
                </p>
            </div>
            {eligibility && !eligibility.canExpose && (
                <div className="rounded-16 bg-gray-1 text-caption-regular text-gray-6 p-12">
                    ...
                    <div className="mt-8 flex flex-col gap-8">
                        {missingRequirementCtas.map(item => (
                            <div
                                ...
                                <p className="text-caption-medium text-primary-600 mt-3">
                                    {item.ctaLabel}
                                </p>
                            </div>
                        ))}
            ...
            {lastResult && (
                <div className="rounded-16 bg-primary-100 border-primary-300 text-caption-regular text-gray-7 border p-12">
                    <p className="text-caption-medium text-gray-8">
                        {lastResult.exposedFriendCount}명의 친구에게 밥약이
                        노출됐습니다.
                    </p>
                    {lastResult.rejectionSummary.length > 0 && (
                        <p className="mt-4">
                            제외된 친구 {lastResult.rejectedFriendCount}명:{' '}
                            {lastResult.rejectionSummary
                                .map(
                                    item =>
                                        `${rejectionReasonLabels[item.reason] ?? item.reason} ${item.count}명`
                                )
                                .join(', ')}
                        </p>
                    )}
            ...
            {errorMessage && (
                <div className="rounded-16 text-caption-regular bg-red-50 p-12 text-red-600">
                    {errorMessage}
                </div>
            )}
            ...
            <button
                ...
                onClick={() => void toggleExposure({ radiusMeters })}
