 ...
 */
export const mealPlanDecisionTaskOrder: Record<MealPlanDecisionTaskKey, number> = {
    SCHEDULE_DATE: 1,
    SCHEDULE_TIME: 2,
...
export const mealPlanDecisionTaskTitleMap: Record<MealPlanDecisionTaskKey, string> = {
...
export const mealPlanDecisionTaskDescriptionMap: Partial<Record<MealPlanDecisionTaskKey, string>> = {
    SCHEDULE_DATE: '함께 시간을 보낼 수 있는 날짜를 모두 골라주세요.',
    SCHEDULE_TIME: '가능한 시간대를 모두 선택해주세요.',
    LOCATION_CANDIDATE: '지도에 위치를 클릭하거나, 장소 검색을 통해 장소를 추가할 수 있어요.',
    ...
    FINAL_CONFIRMATION: '모든 핵심 결정이 준비되면 소유자가 밥약을 확정할 수 있어요.'
}

export const mealPlanDecisionTaskVoteLimitMap: Partial<Record<MealPlanDecisionTaskKey, string>> = {
    SCHEDULE_DATE: '중복 투표',
